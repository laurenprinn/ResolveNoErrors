...
#   .\build-with-toolset.ps1 -Toolset v145    # VS 2026
...
if ([string]::IsNullOrEmpty($CppStandard)) {
	$CppStandard = $info.DefaultCppStd
	Write-Host "Using default C++ standard for $($info.Name): C++$CppStandard" -ForegroundColor Gray
}
...
if ($LASTEXITCODE -eq 0) {
	...
	$exePath = "$buildDir\ResolveNoErrors\$BuildType\ResolveNoErrors.exe"
